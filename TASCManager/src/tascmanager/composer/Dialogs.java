...
public class Dialogs {
    ...
    private static void showDialog(Parent root, final OnShownAction onShown) {
        ...
        dialog.setScene(new Scene(root,  Color.web("#dfdfdf")));
    ...
    private static void showDialogAlert(Parent root, final OnShownAction onShown) {
    ...
    private static void showInformacionWindow(Parent root, final OnShownAction onShown) {
    ...
    public static void showErrorDialog() {
        ...
        try {
            final Parent root = FXMLLoader.load(Dialogs.class.getResource("/tascmanager/view/cuentas_basica_detalle.fxml"));
             //root.getStylesheets().add(Dialogs.class.getResource("styles.css").toExternalForm());
            showDialog(root, new OnShownAction() {
                @Override
                public void perform() {
     ...
     public static void showAlerta() {

        try {
            final Parent roots = FXMLLoader.load(Dialogs.class.getResource("/tascmanager/view/alerta.fxml"));
             //root.getStylesheets().add(Dialogs.class.getResource("styles.css").toExternalForm());
            showDialogAlert(roots, new OnShownAction() {
                @Override
                public void perform() {
                    ...
                    // If I don't do this, the quit button might get the initial focus,
                    // which is not only risky, it looks weird too.
                    for (Node node : roots.lookupAll(".button")) {
                        if (node instanceof Button && ((Button) node).isDefaultButton()) {
                            node.requestFocus();
                        }
                    }
      ...
      public static void showInformacion() {

        try {
            final Parent roots = FXMLLoader.load(Dialogs.class.getResource("/tascmanager/view/informacion.fxml"));
             //root.getStylesheets().add(Dialogs.class.getResource("styles.css").toExternalForm());
            showInformacionWindow(roots, new OnShownAction() {
                @Override
                public void perform() {
